...
### 4. Integrate with your agent workspace

Amenti works best when your agent's core files know about it. Update these files:

- **AGENTS.md** — Add a Memory System section explaining the DB is your brain, MEMORY.md is just an index
- **SOUL.md** — Reference Amenti in your continuity section
- **MEMORY.md** — Convert to a lean scratchpad: Topics table + Active Tasks + Hot Context (< 3k tokens)
- **HEARTBEAT.md** — Add automated cycles: distillation (hourly), reflection (when idle), task checks (30min), task analysis (2h)

See `references/agent-integration.md` for complete templates and examples for each file.

The goal: any future session loads MEMORY.md and immediately knows that real memories are in the database, and this file tells it how to find them.
